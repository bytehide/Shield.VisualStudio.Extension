...
public static class ProjectItemExtensions
{
    ...
    public static ProjectItem FindProjectItem(this ProjectItems items, string filePath)
    {
        ...
        if (findFolder)
        {
            ...
            return (from ProjectItem item in items
                where item.Kind == Constants.vsProjectItemKindVirtualFolder ||
                      item.Kind == Constants.vsProjectItemKindPhysicalFolder
                where folderName == item.Name
                let nextpath = filePath.Substring(backslashIndex + 1)
                select FindProjectItem(item.ProjectItems, nextpath)).FirstOrDefault();
        }
