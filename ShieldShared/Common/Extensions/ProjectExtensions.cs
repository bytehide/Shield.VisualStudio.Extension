...
public static class ProjectExtensions
{
    ...
    public static IEnumerable<string> GetBuildOutputFilePaths(this Project project, BuildOutputFileTypes fileTypes,
        string configuration = null, string platform = null)
    {
        ...
        foreach (var groupName in groups)
        {
            try
            {
                ...
                if (fileUrls != null) filePaths.AddRange(fileUrls.Select(x => new Uri(x.ToString()).LocalPath));
            }
        ...
        try
        {
            ...
            switch (version)
            {
                ...
                default:
                {
                    try
                    {
                        var hexVersion = version.ToString("X");
                        if (hexVersion.Length == 5)
                        {
                            var v1 = int.Parse(hexVersion[0].ToString(CultureInfo.InvariantCulture));
                            var v2 = int.Parse(hexVersion.Substring(1));
    ...
    public static bool IsWixProject(this Project project)
    {
        return project.Kind != null &&
               project.Kind.Equals("{930C7802-8A8C-48F9-8165-68863BCCD9DD}", StringComparison.OrdinalIgnoreCase);
    }
    ...
    {
        // Get the vs solution
        var hr = solution.GetProjectOfUniqueName(project.GetUniqueName(), out var hierarchy);
    ...
    public static async Task<Dictionary<string, string>> GetEvaluatedPropertiesAsync(this Project project)
    {
        try
        {
            ...
            if (context == null)
            {
                ...
                var v = hierarchy.GetProperty((uint)VSConstants.VSITEMID.Root, (int)__VSHPROPID.VSHPROPID_ExtObject,
                    out var extObject);
                if (extObject == null)
                {
                    return [];
                ...
                var dteProject = extObject as Project;
            ...
            if (context is null)
            {
                ...
                if (context == null)
                {
                    var hierarchy = project.ToVsHierarchy(solution);
            ...
            if (context != null)
            {
                ...
                if (configuredProject?.Services.ProjectPropertiesProvider != null)
                {
                    ...
                    return new Dictionary<string, string>
                    {
                        {
                            "ProjectPath",
                            await properties.GetEvaluatedPropertyValueAsync("ProjectPath").ConfigureAwait(false)
                        },
                        {
                            "TargetPath",
                            await properties.GetEvaluatedPropertyValueAsync("TargetPath").ConfigureAwait(false)
                        },
                        {
                            "TargetName",
                            await properties.GetEvaluatedPropertyValueAsync("TargetName").ConfigureAwait(false)
                        },
                        { "TargetExt", await properties.GetEvaluatedPropertyValueAsync("TargetExt").ConfigureAwait(false) },
                        {
                            "TargetFileName",
                            await properties.GetEvaluatedPropertyValueAsync("TargetFileName").ConfigureAwait(false)
                        },
                        { "TargetDir", await properties.GetEvaluatedPropertyValueAsync("TargetDir").ConfigureAwait(false) },
                        {
                            "ProjectDir",
                            await properties.GetEvaluatedPropertyValueAsync("ProjectDir").ConfigureAwait(false)
                        },
                        {
                            "ProjectName",
                            await properties.GetEvaluatedPropertyValueAsync("ProjectName").ConfigureAwait(false)
                        },
                        {
                            "SolutionName",
                            await properties.GetEvaluatedPropertyValueAsync("ProjectDir").ConfigureAwait(false)
                        ...
                        {
                            "SolutionDir",
                            await properties.GetEvaluatedPropertyValueAsync("SolutionDir").ConfigureAwait(false)
                        },
                        {
                            "PlatformName",
                            await properties.GetEvaluatedPropertyValueAsync("ProjectDir").ConfigureAwait(false)
                        ...
                        {
                            "ConfigurationName",
                            await properties.GetEvaluatedPropertyValueAsync("ConfigurationName").ConfigureAwait(false)
                        },
                    };
                }
            }
    ...
    public static string GetLanguageName(this Project project)
    {
        try
        {
            ...
            var codeModelLanguageKind = project.CodeModel.Language;
            return KnownLanguages.ContainsKey(codeModelLanguageKind)
                ? KnownLanguages[codeModelLanguageKind]
    ...
    public static IEnumerable<string> GetFlavourTypes(this Project project)
    {
        try
        {
            ...
            var guidsStr = project.GetProjectTypeGuids();
            if (!string.IsNullOrWhiteSpace(guidsStr))
            {
                var typeGuids = guidsStr.Trim(';').Split(';');
    ...
    public static string GetOutputType(this Project project)
    {
        ...
        try
        {
            var prjObjTypeName = project?.Object?.GetType().Name;
            if (prjObjTypeName is "VCProjectShim" or "VCProject")
            {
                dynamic prj = project.Object;
                var configs = prj.Configurations;
                var config = configs.Item(project.ConfigurationManager.ActiveConfiguration.ConfigurationName);
                string configType = config.ConfigurationType.ToString();

                return configType switch
                {
                    "typeUnknown" => "Unknown",
                    "typeApplication" => "WinExe",
                    "typeDynamicLibrary" => "Library (dynamic)",
                    "typeStaticLibrary" => "Library (static)",
    ...
    {
        return GetKnownProjectType(projectKind) ??
               GetProjectTypeFromRegistry(projectKind, version) ?? UnknownProjectTypeLabel;
    }
    ...
    private static string GetProjectTypeFromRegistry(string projectKind, string version)
    {
        try
        {
            var key = string.Format(@"SOFTWARE\Microsoft\VisualStudio\{0}\Projects\{1}", version, projectKind);
    ...
    {
        var val = project.TryGetPropertyValueOrDefault("RootNamespace") ??
                  project.TryGetPropertyValueOrDefault("DefaultNamespace");
    ...
    public static bool IsDirty(this Project project)
    {
        ...
        if (project.ProjectItems != null &&
            project.ProjectItems.Cast<EnvDTE.ProjectItem>().Any(x => x.ProjectItemIsDirty()))
        {
            return true;
        }
    ...
    public static bool IsHidden(this Project project)
    {
        ...
        try
        {
            ...
            var projectIsInitialized = (project.Object != null);
    ...
    public static Project[] FilterByPaths(Project[] projects, IEnumerable<string> paths)
    {
        ...
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            return lookup.Contains(project.FullName.ToLowerInvariant());
        }).ToArray();
    }
    ...
    {
        var dictByPath = allProjects.ToDictionary(project =>
        {
            ThreadHelper.ThrowIfNotOnUIThread();
            return project.FullName.ToLowerInvariant();
        });
    ...
    public static Dictionary<string, (string, string)[]> GlobalProjectConfigurationProperties(
        this SolutionContexts solutionContexts)
    {
        return solutionContexts
            ...
            }, GlobalProjectConfigurationProperties);
    }
