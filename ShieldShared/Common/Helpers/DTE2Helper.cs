...
public static class Dte2Helper
{
    ...
    public static string GetFullOutputPath(this Project project)
    {
        ...
        try
        {
            ...
            {
                return Path.Combine(
                    Path.GetDirectoryName(project.FullName) ?? throw new InvalidOperationException(),
                    (string)project.ConfigurationManager.ActiveConfiguration.Properties.Item("OutputPath").Value);
            }
            else
            {
                var outputUrlStr =
                    ((object[])project.ConfigurationManager.ActiveConfiguration.OutputGroups.Item("Built").FileURLs)
                    .OfType<string>().First();
